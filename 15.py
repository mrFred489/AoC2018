...
# f = open("15.2.example").read().split("\n")
...
class Unit(object):
    ...
    def iam(self):
        return "Unit(x: {}, y: {}, race: {}, hp: {})".format(self.x, self.y, self.race, self.hp)
    ...
    def copy(self):
        return Unit(self.x, self.y, self.race)
    ...
    while playing:
        ...
        for unit in order:
            ...
            if not len(adjacent) == 0:
                ...
                adjacent.sort()
                if unit.race == "E":
                    adjacent[0][3].hp -= ap_elves
                else:
                    adjacent[0][3].hp -= ap_goblins
                if adjacent[0][3].hp <= 0:
                    ...
                    if sum([1 for x in units if x.race == adjacent[0][3].race]) == 0:
                        if part1:
                            ...
                            if len([x for x in units if x.race == "E"]) == num_elves:
                                print("game over")
                                print("ap_elves", ap_elves)
                                solving = False
                                for k in units:
                                    print(k)
                                print_map(m)
